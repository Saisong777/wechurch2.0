...
export const ParticipantAuth: React.FC<ParticipantAuthProps> = ({ onSuccess, verseReference }) => {
    ...
    <Card className="w-full max-w-md mx-auto border-2">
      <CardHeader className="text-center space-y-4">
        ...
        <div>
          <CardTitle className="text-2xl font-serif">
            {isLogin ? '登入加入查經' : '註冊新帳號'}
          </CardTitle>
          <CardDescription className="text-base mt-2">
            {isLogin ? 'Sign in to join Bible Study' : 'Create account to join'}
          </CardDescription>
          {verseReference && (
      ...
      <CardContent>
        ...
        <Button
          ...
          variant="outline"
          size="lg"
          className="w-full mb-6 h-12 text-base border-2 hover:bg-muted/50"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button
            ...
            disabled={isLoading}
          >
            {isLoading ? '處理中...' : isLogin ? '登入 Sign In' : '註冊 Sign Up'}
          </Button>
        </form>

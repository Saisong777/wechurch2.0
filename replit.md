...
- **We Play**: Parent section at `/play` containing two sub-features: "真心話不用冒險" (icebreaker card game at `/icebreaker`) and "只能說是神的安排" (random grouper at `/grouper`). Navigation shows a single "We Play" button that links to the landing page with both feature cards.
- **Random Grouper ("只能說是神的安排")**: Enables leaders to create temporary, code-based grouping activities with real-time participant updates and privacy-focused data handling.
...
- **Admin Mail System (信件系統)**: Admin/leader-only email composer (`AdminMailComposer.tsx`) accessible from admin dashboard. Supports 4 recipient modes: all users, by role (admin/leader/member), by church, or individual selection with searchable checkboxes. Uses TipTap rich text editor for HTML email body. Sends via existing Resend integration (`server/resend.ts`). Backend endpoints: `GET /api/admin/users-for-email` (auth-guarded, returns users with email/role/church), `POST /api/send-bulk-email` (auth-guarded). Shows recipient count preview and send result summary. Supports file attachments (up to 5 files, 10MB each) via base64 encoding. RichTextEditor toolbar uses `onMouseDown` with `preventDefault` to maintain editor focus during formatting. **Inbox (收件匣)**: `AdminInbox.tsx` component with list/detail views for receiving reply emails. Inbound emails stored in `inbox_emails` table. Webhook endpoint `POST /api/webhooks/resend/inbound` receives emails from Resend (optional secret via `RESEND_WEBHOOK_SECRET` env var). Admin endpoints: `GET /api/admin/inbox`, `GET /api/admin/inbox/unread-count`, `PATCH /api/admin/inbox/:id/read`, `PATCH /api/admin/inbox/:id/archive`. Outgoing emails include `reply_to: reply@wechurch.online` header. HTML content sanitized with DOMPurify before rendering. Dashboard shows unread count badge on inbox card.

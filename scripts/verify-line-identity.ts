...
export async function verifyLineIdentity(pool: Pool) {
  ...
  const providerBefore = process.env.LINE_PROVIDER_ID;
  const channelsBefore = process.env.LINE_PROVIDER_CHANNELS;
  try {
    process.env.LINE_PROVIDER_ID = 'synthetic-provider';
    process.env.LINE_PROVIDER_CHANNELS = JSON.stringify({ 'synthetic-channel': 'synthetic-provider', 'other-channel': 'synthetic-provider' });
    const rolesBefore = (await pool.query('SELECT role FROM user_roles WHERE user_id=$1 ORDER BY role', [first.userId])).rows;
    assert.equal((await ensureLineLinkedUser({ ...profile, channelId: 'other-channel' })).userId, first.userId);
    assert.deepEqual((await pool.query('SELECT role FROM user_roles WHERE user_id=$1 ORDER BY role', [first.userId])).rows, rolesBefore);
  } finally {
    if (providerBefore === undefined) delete process.env.LINE_PROVIDER_ID; else process.env.LINE_PROVIDER_ID = providerBefore;
    if (channelsBefore === undefined) delete process.env.LINE_PROVIDER_CHANNELS; else process.env.LINE_PROVIDER_CHANNELS = channelsBefore;
  }

...
for (const url of ['/api/dev-login', '/api/login', '/api/cron/daily-follow-email', '/api/webhooks/resend']) assert.equal((await call(url)).status, 403);
if (app.STAGING_LINE_LOGIN_ENABLED === '1') assert.equal((await data(await call('/api/line-login/config'))).configured, true);
else assert.equal((await call('/api/line-login/url')).status, 403);

...
// IMPORTANT: Use Arabic numerals (1, 2, 3) not Chinese numerals (一, 二, 三) for group numbers
const SYSTEM_PROMPT_GROUP = `你是「共同查經小組分析助理」。你的工作是**忠實整理**筆記，**絕對禁止**添加任何內容。

🚨 嚴格禁止：❌添加筆記沒有的神學/經文 ❌潤飾美化 ❌編造無人說過的話 ❌推測延伸 ❌空洞形容詞（深刻、寶貴、美好） ❌使用亂碼或不可讀字元
...
3) 內容簡短就直接說，不要編造填充
4) 列出每位成員，沒寫內容就標「未填寫」
5) ⚠️ 組別必須使用阿拉伯數字：「第 1 組」「第 2 組」（不可用「第一組」「第二組」）
6) 如果筆記內容是測試文字或亂碼（如 "test", "asdf", "sdafsdafasdfasdfsadf"），直接標記為「測試內容」，不要原樣輸出
...
// Overall report prompt: Cross-group synthesis, NOT single group analysis
const SYSTEM_PROMPT_OVERALL = `你是「全會眾查經綜合分析助理」。你的任務是**跨組綜合分析**所有小組的查經筆記，找出共同主題和不同觀點。

🚨 嚴格禁止：❌添加筆記沒有的神學/經文 ❌潤飾美化 ❌編造無人說過的話 ❌推測延伸 ❌空洞形容詞

✅ 必須遵守：
1) 分析**所有組別**的筆記，不是只看第一組
2) 找出跨組共識：多組都提到的主題或觀點
3) 呈現多元觀點：不同組別的獨特發現
4) 個人亮光必須標註姓名和組別：「👤 [姓名]（第 N 組）：[內容]」
5) 測試文字或亂碼標記為「測試內容」，不納入分析
6) 統計各組參與人數

格式（嚴格遵守）：
**📊 全會眾綜合分析**
**參與組別：** 共 N 組（第1組: X人、第2組: Y人...）
**總參與人數：** X 人
**查經經文：** 經文
---
**🎯 跨組共識（多組提及的主題）：**
...
serve(async (req) => {
  ...
  try {
    ...
    // Calculate group statistics for overall report
    const groupStats = new Map<number, { count: number; names: string[] }>();
    allData.forEach(d => {
      const grp = d.組別 || 0;
      if (!groupStats.has(grp)) {
        groupStats.set(grp, { count: 0, names: [] });
      }
      const stat = groupStats.get(grp)!;
      stat.count++;
      if (d.姓名) stat.names.push(d.姓名);
    });
    
    const groupCount = groupStats.size;
    const groupSummary = Array.from(groupStats.entries())
      .sort((a, b) => a[0] - b[0])
      .map(([grp, stat]) => `第${grp}組: ${stat.count}人`)
      .join("、");

    // Different prompts for group vs overall reports
...
請依照指定格式生成完整報告，確保涵蓋所有 ${totalMembers} 位成員的貢獻。`;
      ...
      systemPrompt = SYSTEM_PROMPT_OVERALL;
      userPrompt = `請根據以下**所有組別**的查經筆記，生成一份「全會眾綜合分析報告」。

⚠️ 重要提醒：
- 這是**跨組綜合分析**，不是單一組的報告
- 共有 ${groupCount} 個組別，總計 ${totalMembers} 位成員
- 各組人數：${groupSummary}
- 成員名單：${memberNames}
- 請分析**所有組別**的筆記，找出共同主題和不同觀點${unfilledNote}

經文：${verseRef}

所有組別的查經筆記資料（共 ${totalMembers} 份）：
${compactEntries}

請依照指定格式生成跨組綜合分析報告，確保涵蓋所有 ${groupCount} 個組別、${totalMembers} 位成員的貢獻。`;
    }

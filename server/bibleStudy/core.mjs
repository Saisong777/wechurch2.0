...
// Adapter for public-20260925-v1. Text, source metadata and the delivered DB are unchanged.
...
export const translations = { cmncbt: 'Biblica® 當代譯本開放資源（繁體）', engwebp: 'World English Bible' };
...
export function createReader(filename) {
  ...
  return {
    ...
    query(action, q = {}) {
      ...
      const sid = q.translation || 'cmncbt';
      if (!Object.hasOwn(translations, sid)) invalid();
      if (action === 'info') return { books, translations, sources, release_id: 'public-20260925-v1', copyright_url: '/open/licenses', note_sources: noteSources };
      if (action === 'item') {

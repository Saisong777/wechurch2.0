import { pool } from "./db";
import { lineChannelContinuity } from './lineLoginPolicy';
...
export async function ensureLineLinkedUser(profile: LineVerifiedProfile, linkUserId?: string): Promise<LineLinkedUser> {
  ...
  if (!lineChannelContinuity(bound?.channel_id, profile.channelId)) throw new Error('LINE channel binding mismatch');

...
export function lifeGroupRoutes(resolveUserId: (req: Request) => Promise<string | null>) {
  ...
  router.post('/:groupId/invite', async (_req, res) => {
    const invite = await groups.rotateInvite(...args(res));
    const fragment = new URLSearchParams({ group: invite.token });
    const staging = isTestDeployment();
    if (staging) {
      const code = process.env.STAGING_ACCESS_CODE;
      if (!code || code.length < 16) throw new groups.GroupError(503, '測試邀請尚未設定。');
      fragment.set('ticket', stagingTicket(code, Date.now(), 7));
    }
    const url = staging ? `/__staging/invite#${fragment}` : `/groups#invite=${invite.token}`;
    res.json({ ...invite, url });
  });

...
export function stagingTicket(code: string, now = Date.now(), days = 30) {
  if (!Number.isInteger(days) || days < 1 || days > 30) throw new Error('Invalid invitation duration');
  const expiry = String(now + days * 86400_000);
...
export function stagingAccessGate(env: Env = process.env): RequestHandler {
  ...
  return (req, res, next) => {
    ...
    // Same-origin forms need a non-null Origin; fragments never enter referrers.
    ...
    if (req.path === '/__staging/invite' && req.method === 'GET') return res.type('html').send(invitationLanding);
    if (req.path === '/__staging/invite.js' && req.method === 'GET') return res.type('application/javascript').send(invitationScript);
    if (req.path === '/__staging/access' && req.method === 'POST') {
      ...
      const invited = typeof req.body?.ticket === 'string' && validStagingTicket(req.body.ticket, code, now);
      if (!invited && (typeof req.body?.code !== 'string' || !equal(req.body.code, code))) return res.status(401).type('html').send(invitationPage('邀請碼不正確或連結已過期，請向邀請人索取新連結。'));
      attempts.delete(ip);
      const granted = invited ? req.body.ticket as string : stagingTicket(code, now);
      res.cookie('__Host-wechurch-staging', granted, { httpOnly: true, secure: true, sameSite: 'lax', path: '/', maxAge: Number(granted.split('.')[0]) - now });
      const group = typeof req.body?.group === 'string' && /^[a-f0-9]{48}$/.test(req.body.group) ? req.body.group : '';
      return res.redirect(303, group ? `/groups#invite=${group}` : '/login?mode=signup');
    }

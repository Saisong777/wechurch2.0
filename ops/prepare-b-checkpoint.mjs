...
const git=(args,input,env={})=>execFileSync('git',args,{cwd:root,input,encoding:'utf8',maxBuffer:32*1024*1024,env:{...process.env,...env}}).trim();
...
for(const file of ['.gitignore','.env.example','ops/verify-b-checkpoint.mjs','ops/verify-b-checkpoint.test.mjs','ops/prepare-b-checkpoint.mjs','design/staging-encrypted-backup-2026-09-13.md','design/b-checkpoint-readme-2026-09-13.md'])add(file,fs.readFileSync(path.join(root,file)));

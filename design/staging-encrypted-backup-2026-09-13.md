...
目前研經資料較大，改用 `ops/seal-b-recovery-set.mjs` 的 format 2：
DB、私人上傳檔、設定、驗證證據與只讀研經資料五份分開加密。
還原用 `ops/verify-b-recovery-set.mjs`，已通過 96 表逐表內容摘要核對。
本輪已獲准上傳 Google Drive 私人資料夾，進度、下載後還原與金鑰保管邊界
見 [新譯本及備份驗收](bible-feb-backup-2026-09-26.md)。沒有自動排程。

下方為原 format 1 三檔工具的歷史操作，不能用來取代現役 format 2 的完整研經資料備份。
